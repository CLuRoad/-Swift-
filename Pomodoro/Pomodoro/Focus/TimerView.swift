...
//        let format = "02"
        let labelText = String(format:"%02x",minutes) + ":" + String(format:"%02x",seconds)
        
        timeLabel.text = labelText
        
        let totalMinutes = (maxValue-1) / 60
        let dashLength = 2*radius*CGFloat(Double.pi)/totalMinutes
    ...
    private func settingSecondsRingStyle() {
        ...
        secondsShapeLayer.strokeColor = TimeStyleKit.timerColor.cgColor
        secondsShapeLayer.lineWidth = 1.0
    }
    
    private func settingFullRingStyle() {
        TimeStyleKit.timerColor.set()
